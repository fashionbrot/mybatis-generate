...
public class OracleQuery extends AbstractQuery{
    ...
    public String tablesListSql(String tableName) {
        ...
        sql.append("SELECT DT.TABLE_NAME as tableName, DTC.COMMENTS as comments, UO.CREATED as createTime, UO.LAST_DDL_TIME as updateTime ");
        sql.append("FROM USER_TABLES DT ");
        sql.append("INNER JOIN USER_TAB_COMMENTS DTC ON DT.TABLE_NAME = DTC.TABLE_NAME ");
        sql.append("INNER JOIN USER_OBJECTS UO ON DT.TABLE_NAME = UO.OBJECT_NAME ");
        sql.append("WHERE UO.OBJECT_TYPE = 'TABLE' ");
        if (ObjectUtil.isNotEmpty(tableName)){
            sql.append(" AND DT.TABLE_NAME LIKE '%"+tableName.toUpperCase()+"%'  ");
        }
        sql.append("ORDER BY DT.TABLE_NAME");
        return sql.toString();
    }
    ...
    public String tablesSql(String tableName) {
        StringBuilder sql =new StringBuilder();
        sql.append("SELECT DT.TABLE_NAME as tableName, DTC.COMMENTS as comments, UO.CREATED as createTime ");
        sql.append("FROM USER_TABLES DT ");
        sql.append("INNER JOIN USER_TAB_COMMENTS DTC ON DT.TABLE_NAME = DTC.TABLE_NAME ");
        sql.append("INNER JOIN USER_OBJECTS UO ON DT.TABLE_NAME = UO.OBJECT_NAME ");
        sql.append("WHERE UO.OBJECT_TYPE = 'TABLE' ");
        sql.append("AND DT.TABLE_NAME = '"+tableName.toUpperCase()+"'");
        return sql.toString();
    }
    ...
    public String tableFieldsSql(String tableName) {
        ...
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT TEMP.TABLE_NAME     as tableName, ");
        sql.append("TEMP.COLUMN_ID as columnId, ");
        sql.append("TEMP.COLUMN_NAME as columnName, ");
        sql.append("TEMP.DATA_TYPE      as dataType, ");
        sql.append("TEMP.DATA_LENGTH    as dataLength, ");
        sql.append("TEMP.DATA_PRECISION as dataPrecision, ");
        sql.append("TEMP.DATA_SCALE     as dataScale, ");
        sql.append("TEMP.NULLABLE       as nullable, ");
        sql.append("TEMP.COMMENTS       as comments, ");
        sql.append("CASE TEMP.CONSTRAINT_TYPE  WHEN 'P' THEN 'PRI' WHEN 'C' THEN 'UNI' ELSE '' END  \"columnKey\", '' \"extra\" ");
        sql.append("FROM (  ");
        sql.append("SELECT COL.TABLE_NAME,COL.COLUMN_ID,COL.COLUMN_NAME,COL.DATA_TYPE,COL.DATA_LENGTH,COL.DATA_PRECISION,COL.DATA_SCALE,COL.NULLABLE,COLC.COMMENTS,UC.CONSTRAINT_TYPE, ");
        sql.append("ROW_NUMBER() OVER (PARTITION BY COL.COLUMN_NAME ORDER BY UC.CONSTRAINT_TYPE DESC) AS ROW_FLG ");
        sql.append("FROM USER_TAB_COLUMNS COL  ");
        sql.append("LEFT JOIN USER_COL_COMMENTS COLC ON COLC.TABLE_NAME = COL.TABLE_NAME AND COLC.COLUMN_NAME = COL.COLUMN_NAME ");
        ...
        sql.append("WHERE COL.TABLE_NAME = '"+tableName.toUpperCase()+"' ) TEMP ");
        sql.append("WHERE TEMP.ROW_FLG = 1 ORDER BY TEMP.COLUMN_ID ");
